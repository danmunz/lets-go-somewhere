...
const progressMessage = (count: number) => count < 8 ? 'Finding your trip rhythm.' : count < 18 ? 'You’re on a roll.' : count < 24 ? 'Almost there — just a few gut calls left.' : 'Checking the final shape.';
...
function App() {
  ...
  const handleRouteError = useCallback(async (reason: unknown, source: Parameters<typeof routeIntentForApiError>[1]) => {
    ...
    else if (intent === 'show-waiting') { setError('The envelope is still sealed. You can check who is ready.'); setScreen('waiting'); }
  ...
  const progress = next && !next.complete ? next.progress : { comparisons: 0, minimum: 24, maximum: 40, estimatedCompletion: 0 };
