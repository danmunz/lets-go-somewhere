...
The original group reveal is complete and remains immutable. A separate direct-destination Lightning Round is deployed for the group’s follow-up decision: 24 researched destination briefs, 48 fair-coverage comparisons plus at most 12 deterministic tie-breakers, compact direct-choice cards, private full rankings with a caller-only decision trail, a required zero-to-four advisory-veto step, a departure-board waiting state, second-envelope gating, and a transparent 24-to-1 Borda group board with a sortable five-person rank table. A saved veto tells the group that its owner would not take that trip; it does not alter model ranks, Borda points, or group order. The deployed `working-order-borda-v2` contract replaces all-way shared tiers with an exact current order, private 4,096-draw top-five/rank-range evidence, and 75% clear-break markers; the group result exposes qualitative bands only. It uses isolated content versioning and `lgsV4Lightning*` Firestore collections, so it cannot change original comparisons or their snapshot.
...
- Commit `60eb326` was deployed to Cloud Run revision `lgs-api-00015-92r` and Firebase Hosting. The service routes 100% of traffic to that revision; the public health endpoint and Hosting URL returned successfully. Read-only production rendering checks confirmed the sealed Lightning waiting view (five of five saved) and an authenticated caller’s actual private list, vetoes, and evidence labels. No envelope, reset, or participant-data write was invoked.
...
The original one-trip release was deployed on 2026-08-22 as Cloud Run revision `lgs-api-00009-x8r` and Firebase Hosting version `ab1c50b7b1550cc8`. The current production release is commit `60eb326`, deployed on 2026-08-24 as Cloud Run revision `lgs-api-00015-92r` and Firebase Hosting. The API health route and public Hosting route returned successfully. The original trip has now started and its reveal is open, so the historical empty-state preflight does not apply to this isolated Lightning follow-up deployment.
