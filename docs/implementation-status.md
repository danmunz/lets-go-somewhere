...
2. **Rehearsal — passed:** The isolated Auth/Firestore Emulator verifies configuration, concurrent/stale pending claims, racing immutable reveal creation, persisted snapshot reload, immutable final-decision conflicts, redacted v2-result serialization, and five disposable authenticated identities through mismatch recovery, resume/duplicate handling, all five exact 32-choice rounds, completion gates, Dan-only reveal, immutable snapshot parity, and stale-tab final-decision behavior. A literal browser pass independently verified character selection, local authentication, the required briefing, all 32 choices for one disposable traveler, profile, private shortlist, atlas/map/list synchronization, photo viewer, and crew-status gate. The five-person browser state was exercised through the authenticated API emulator rehearsal, not five separate visual-browser contexts.
3. **Production verification — passed:** Fixture-based desktop/mobile visual and accessibility review is complete locally. The limited disposable cloud smoke passed and the temporary project is deletion-requested. Cloud Run revision `lgs-api-00007-nfn` and Firebase Hosting were deployed successfully. Health and public-route checks passed, and the final count-only production preflight returned `empty: true` with zero started or completed travelers, snapshots, and decisions. Production must never be used for a test journey because the guarded reset intentionally refuses started state.
