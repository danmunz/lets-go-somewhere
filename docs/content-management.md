...
Most route copy is colocated in `frontend/src/main.tsx`. The first-run briefing is intentionally its own screen in `frontend/src/screens/HowItWorksScreen.tsx`; its entry/hash/back helpers are in `frontend/src/howItWorks.ts`. The comparison progress milestones are the `progressMessage` function near the top of `main.tsx`. Profile and private-shortlist copy are in `frontend/src/screens/ProfileScreen.tsx` and `MyResultsScreen.tsx`; Atlas header, selection, gallery, lightbox, and fallback copy are in `frontend/src/AtlasExplorer.tsx` and `AtlasMap.tsx`; waiting copy is in `WaitingScreen.tsx`; verdict and final-decision copy are in `VerdictScreen.tsx`, `VerdictAdditions.tsx`, and `FinalDecisionDialog.tsx`. Update the relevant focused test when copy is asserted exactly.
