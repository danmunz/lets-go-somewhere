...
| Lightning Round cards, veto step, help, and second-envelope copy | `frontend/src/screens/LightningScreens.tsx`, `frontend/src/components/JourneyNav.tsx`, `frontend/src/journeyNavigation.ts`, and `frontend/src/app.css` | Direct-card detail, fixed 48-core/up-to-12-tie-breaker explanation, private full-list and its caller-only decision trail, the up-to-four advisory-veto step, `About this round`, departure-board waiting state, and sortable group-rank table language. Keep the focus header during direct choices/pending vetoes; after vetoes are saved use the Lightning navigator and explicit round switcher. Do not reuse this visible destination data in blind comparison UI. |
