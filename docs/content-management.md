...
| Profile, waiting, results, and verdict copy | `frontend/src/screens/` and `frontend/src/components/` | Screen-specific post-completion and post-reveal language. `JourneyNav.tsx` and `journeyNavigation.ts` own durable navigation labels, descriptions, hashes, and phase eligibility; raw activity choices must never be rendered here. Use the approved plain-language vocabulary below. |
...
Most route copy is colocated in `frontend/src/main.tsx`. The first-run briefing and the contextual `? How it works` control are in `frontend/src/screens/HowItWorksScreen.tsx`; its entry/hash/back helpers are in `frontend/src/howItWorks.ts`. The floater belongs only to signed-in comparison play and the completion handoff. The comparison progress milestones are the `progressMessage` function near the top of `main.tsx` and must stay truthful to the fixed 32-choice round. Profile and private-top-five copy are in `frontend/src/screens/ProfileScreen.tsx` and `MyResultsScreen.tsx`; `frontend/src/components/JourneyNav.tsx` renders the post-completion desktop bar and mobile sheet, while `frontend/src/journeyNavigation.ts` owns the labels, descriptions, hashes, and reveal eligibility. Atlas header, selection, gallery, lightbox, and fallback copy are in `frontend/src/AtlasExplorer.tsx` and `AtlasMap.tsx`; waiting copy is in `WaitingScreen.tsx`; verdict copy and its closing line are in `VerdictScreen.tsx` and `VerdictAdditions.tsx`. Update the relevant focused test when copy is asserted exactly.
