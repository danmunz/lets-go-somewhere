  ...
  useEffect(() => {
    ...
    try {
      ...
      instance.on('load', () => {
        ...
        instance.addLayer({ id: 'destination-points', type: 'circle', source: 'destinations', filter: ['!', ['has', 'point_count']], paint: { 'circle-color': '#d4924d', 'circle-radius': 13, 'circle-stroke-color': '#211c17', 'circle-stroke-width': 5, 'circle-opacity': .98 } });
        instance.addLayer({ id: 'destination-labels', type: 'symbol', source: 'destinations', minzoom: 4, filter: ['!', ['has', 'point_count']], layout: { 'text-field': ['get', 'name'], 'text-font': ['Open Sans Bold'], 'text-size': 15, 'text-offset': [0, 1.55], 'text-anchor': 'top' }, paint: { 'text-color': '#211c17', 'text-halo-color': '#f8f0e5', 'text-halo-width': 2 } });
    ...
    return () => { observer?.disconnect(); map.current?.remove(); map.current = undefined; };
