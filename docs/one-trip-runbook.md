...
**Status:** released 2026-08-22. The isolated Auth/Firestore Emulator transaction suite and authenticated five-identity API rehearsal pass locally. A literal browser pass covered a disposable traveler’s character selection, local sign-in, briefing, all 32 blind comparisons, profile, private shortlist, atlas/map/list/photo viewer, and crew-status gate. A disposable Cloud Run/Firebase/Firestore project passed health, Hosting routing, disposable approved-account authentication, one Firestore-backed blind comparison, response redaction, and count-only state inspection before being deletion-requested. Firebase Hosting version `ab1c50b7b1550cc8` and Cloud Run revision `lgs-api-00009-x8r` are live; health, public Hosting, and the pre/post-deploy production preflights passed. This document remains the operating procedure; no real trip has started.
