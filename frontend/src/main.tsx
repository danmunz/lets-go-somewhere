...
import { AppStateNotice, CompletedTransition, JourneyNav, LightningFocusHeader, LightningNav, MediaImage, TravelEffortKey } from './components/index.js';
...
function App() {
  ...
  const setLightningHash = useCallback((destination: LightningDestination, replace = false) => {
    const hash = lightningHashFor(destination);
    if (window.location.hash === hash) return;
    handledJourneyHash.current = hash;
    window.history[replace ? 'replaceState' : 'pushState'](null, '', hash);
  }, []);
  ...
  const refreshLightningRoundStatus = useCallback(async () => {
    if (!api) return undefined;
    try { const response = await api.getLightningStatus(); setLightningStatus(response); return response; }
    catch { return undefined; }
  }, [api]);
  useEffect(() => {
    if (api && profile && (Boolean(results) || Boolean(status?.revealOpen))) void refreshLightningRoundStatus();
  }, [api, profile, refreshLightningRoundStatus, results, status?.revealOpen]);
  const openLightning = async (updateHash = true) => {
    if (!api) return; setBusy(true); setError('');
    try {
      const round = await api.getLightningStatus(); setLightningStatus(round);
      if (round.revealOpen) { setLightningGroupResults(await api.getLightningGroupResults()); setScreen('lightning-verdict'); if (updateHash) setLightningHash('verdict'); }
      else if (round.rankingComplete) { setLightningPersonal(await api.getLightningPersonalResults()); setScreen('lightning-results'); if (updateHash) setLightningHash('list'); }
      else if (round.progress.comparisons > 0) {
        const nextRound = await api.getNextLightningComparison(); setLightningNext(nextRound); setScreen(nextRound.complete ? 'lightning-results' : 'lightning-comparison'); if (updateHash) setLightningHash(nextRound.complete ? 'list' : 'entry');
      } else { setScreen('lightning-intro'); if (updateHash) setLightningHash('entry'); }
    } catch (reason) { await handleRouteError(reason, 'lightning'); } finally { setBusy(false); }
  };
  const loadLightningComparison = async () => { if (!api) return; setBusy(true); try { const response = await api.getNextLightningComparison(); setLightningNext(response); if (response.complete) { setLightningPersonal(await api.getLightningPersonalResults()); setScreen('lightning-results'); setLightningHash('list'); } else { setScreen('lightning-comparison'); setLightningHash('entry', true); } } catch (reason) { await handleRouteError(reason, 'lightning'); } finally { setBusy(false); } };
  ...
  const openLightningWaiting = async (updateHash = true) => { const state = await refreshLightningStatus(); if (state) { if (state.revealOpen) { await openLightningVerdict(updateHash); return; } setScreen('lightning-waiting'); if (updateHash) setLightningHash('waiting'); } };
  const revealLightning = async () => { if (!api) return; setBusy(true); try { await api.openLightningReveal(); setLightningGroupResults(await api.getLightningGroupResults()); setScreen('lightning-verdict'); setLightningHash('verdict'); } catch (reason) { await handleRouteError(reason, 'lightning'); } finally { setBusy(false); } };
  const openLightningVerdict = async (updateHash = true) => { if (!api) return; setBusy(true); try { setLightningGroupResults(await api.getLightningGroupResults()); setScreen('lightning-verdict'); if (updateHash) setLightningHash('verdict'); } catch (reason) { await handleRouteError(reason, 'lightning'); } finally { setBusy(false); } };
  ...
  const navigateLightning = useCallback(async (destination: LightningDestination, updateHash = true) => {
    if (!api) return;
    if (destination === 'entry') { await openLightning(updateHash); return; }
    setBusy(true); setError('');
    try {
      const round = await api.getLightningStatus();
      setLightningStatus(round);
      if (destination === 'help') {
        lightningHelpReturnRef.current = screenRef.current;
        setLightningHelpReturn(screenRef.current);
        setScreen('lightning-help');
        if (updateHash) {
          const hash = lightningHashFor('help');
          handledJourneyHash.current = hash;
          window.history.pushState({ lgsLightningHelp: true }, '', hash);
        }
        return;
      }
      if (!round.rankingComplete) { await openLightning(updateHash); return; }
      if (destination === 'veto' && !round.vetoSubmitted) {
        setLightningPersonal(await api.getLightningPersonalResults());
        setScreen('lightning-veto');
        if (updateHash) setLightningHash('veto');
        return;
      }
      if (destination === 'veto' || destination === 'list') {
        setLightningPersonal(await api.getLightningPersonalResults());
        setScreen('lightning-results');
        if (updateHash) setLightningHash('list');
        return;
      }
      if (!round.vetoSubmitted) {
        setError('Save your vetoes before checking who’s ready.');
        setLightningPersonal(await api.getLightningPersonalResults());
        setScreen('lightning-results');
        if (updateHash) setLightningHash('list');
        return;
      }
      if (destination === 'waiting') { await openLightningWaiting(updateHash); return; }
      if (!round.revealOpen) {
        setError('The second envelope is still sealed. You can see who’s ready here.');
        await openLightningWaiting(updateHash);
        return;
      }
      await openLightningVerdict(updateHash);
    } catch (reason) { await handleRouteError(reason, 'lightning'); }
    finally { setBusy(false); }
  }, [api, handleRouteError, openLightning, openLightningVerdict, openLightningWaiting, setLightningHash]);
  ...
  const closeLightningHelp = useCallback(() => {
    if (window.location.hash === lightningHashFor('help') && window.history.state?.lgsLightningHelp) window.history.back();
    else void openLightning(true);
  }, [openLightning]);
  useEffect(() => {
  ...
  useEffect(() => {
    ...
    const restoreFromHash = () => {
      ...
      const lightningDestination = lightningDestinationFromHash(window.location.hash);
      if (lightningDestination) {
        void navigateLightning(lightningDestination, false);
        return;
      }
  ...
  }, [api, navigateJourney, navigateLightning, profile, setJourneyHash]);
  ...
  const journeyNav = user && profile && currentJourney
    ? <JourneyNav active={currentJourney} revealOpen={Boolean(results || status?.revealOpen)} revealSeen={revealSeen} onNavigate={(destination) => navigateJourney(destination)} onOpenHowItWorks={() => openHowItWorks()} onOpenLightning={Boolean(results || status?.revealOpen) ? () => void openLightning() : undefined} />
    : null;
  const currentLightning = lightningNavigationDestinationForScreen(screen);
  const lightningNavigationReady = screen === 'lightning-waiting' || screen === 'lightning-verdict' || (screen === 'lightning-results' && Boolean(lightningPersonal?.vetoes.submitted));
  const lightningNav = user && currentLightning && lightningNavigationReady
    ? <LightningNav active={currentLightning} revealOpen={Boolean(lightningGroupResults || lightningStatus?.revealOpen || lightningGroupStatus?.revealOpen)} onNavigate={(destination) => void navigateLightning(destination)} onOpenRoundOne={() => navigateJourney('verdict')} onOpenLightning={() => void openLightning()} onOpenHelp={() => void navigateLightning('help')} />
    : null;
  const lightningFocusStatus = screen === 'lightning-intro'
    ? 'Ready to begin'
    : screen === 'lightning-comparison' && lightningNext
      ? lightningNext.progress.phase === 'tie-breakers'
        ? `${lightningNext.progress.comparisons} choices · close calls`
        : `${lightningNext.progress.comparisons} of ${lightningNext.progress.core} choices`
      : 'Choose your vetoes';
  const lightningFocusHeader = user && (screen === 'lightning-intro' || screen === 'lightning-comparison' || screen === 'lightning-veto' || (screen === 'lightning-results' && !lightningPersonal?.vetoes.submitted))
    ? <LightningFocusHeader status={lightningFocusStatus} onOpenRoundOne={() => navigateJourney('verdict')} />
    : null;
  ...
  if (screen === 'lightning-intro') return <>{lightningFocusHeader}<LightningIntroScreen onStart={() => void loadLightningComparison()} />{notice}</>;
  if (screen === 'lightning-comparison' && lightningNext && !lightningNext.complete) return <>{lightningFocusHeader}<LightningComparisonScreen progress={lightningNext.progress} destinations={[lightningNext.destinationA, lightningNext.destinationB]} selected={lightningPicked} onChoose={(winner) => void chooseLightning(winner)} />{notice}</>;
  if (screen === 'lightning-results' && lightningPersonal) return <>{lightningNav ?? lightningFocusHeader}<LightningPersonalResultsScreen results={lightningPersonal} onOpenWaiting={() => void openLightningWaiting()} onOpenVeto={() => void navigateLightning('veto')} />{notice}</>;
  if (screen === 'lightning-veto' && lightningPersonal) return <>{lightningFocusHeader}<LightningVetoScreen results={lightningPersonal} onSubmit={submitLightningVetoes} />{notice}</>;
  if (screen === 'lightning-waiting' && lightningGroupStatus && user) return <>{lightningNav}<LightningWaitingScreen status={lightningGroupStatus} user={user} onRefresh={() => void refreshLightningStatus()} onReveal={() => void revealLightning()} onOpenResults={() => void openLightningVerdict()} />{notice}</>;
  if (screen === 'lightning-verdict' && lightningGroupResults) return <>{lightningNav}<LightningVerdictScreen results={lightningGroupResults} />{notice}</>;
  if (screen === 'lightning-help') return <LightningHowItWorksScreen backLabel={lightningHelpReturn === 'lightning-waiting' ? 'Back to who’s ready' : lightningHelpReturn === 'lightning-verdict' ? 'Back to how everyone ranked' : 'Back to my full list'} onBack={closeLightningHelp} />;
