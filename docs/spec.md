...
- `bayes-direct-destination-v1` is a server-owned Bayesian Bradley–Terry model. It uses deterministic posterior draws to select the most useful remaining direct comparison. The private result is an exact current order, with a clearly labeled break only when an adjacent ordering clears a 75% posterior check; its private evidence panel shows a top-five chance and plausible rank range for each place. It does not turn uncertainty into a forced 24-place shared tier.
...
- A veto is an advisory dealbreaker signal, not a scoring rule. It does not change the Bayesian direct ranking, personal order, 24-to-1 Borda points, or group ordering.
- The opened group view applies a transparent 24-to-1 Borda tally: rank 1 earns 24 points and rank 24 earns 1. It exposes every traveler’s complete direct list as an accessible five-column rank table: the group can sort by any traveler or select a place to compare all five ranks. A submitted veto appears as a clearly labeled red X instead of that traveler’s displayed rank, and each affected group-list row names the travelers who vetoed it. The shared result may show qualitative “more often near the top” and “could still move up” bands, but never another traveler’s numeric posterior evidence. It is a conversation guide, not an automatic trip decision.
