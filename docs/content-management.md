...
| Profile, waiting, results, verdict, and decision copy | `frontend/src/screens/` and `frontend/src/components/` | Screen-specific post-completion and post-reveal language. `JourneyNav.tsx` owns durable navigation labels; raw activity choices must never be rendered here. Use the approved plain-language vocabulary below. |
