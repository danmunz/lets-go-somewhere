...
export function AtlasExplorer({ destinations, user, travelerName, avatarSrc, onOpenWaiting, onOpenProfile }: Props) {
  ...
  return <main className="atlas-explorer screen-enter" aria-labelledby="atlas-title">
    <header className="atlas-explorer__header"><div className="atlas-explorer__brand"><img src={logoUrl} alt="Let's Go Somewhere" /><div><p className="eyebrow">All 24 places</p><strong id="atlas-title">{candidateCount} possible trips. None is ranked here.</strong></div></div><p className="atlas-explorer__purpose">Explore every place that was in the running. This is not your ranking.</p><nav className="atlas-explorer__nav" aria-label="Finished trip navigation">{onOpenProfile && <button onClick={onOpenProfile}>What I liked</button>}<button className="is-active" aria-current="page">All 24 places</button><button onClick={onOpenWaiting}>Who’s finished</button></nav><div className="atlas-explorer__traveler">{avatarSrc && <img src={avatarSrc} alt="" />}<span>{travelerName(user)}</span></div></header>
