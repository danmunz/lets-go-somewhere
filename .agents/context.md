...
- The independent transparent-social-ballot audit (`fbae847`), guarded count-only preflight/reset implementation (`4ce95f0`), seven-case Firestore Emulator persistence proof (`a785de3`), and fixture-based desktop/mobile visual/accessibility review (`3f101e4`, `06551f8`) have passed locally. They do not replace fixed-shortlist verification, the five-identity browser rehearsal, or disposable-environment smoke.
...
- Browser-harness is connected for literal local visual review. It verified the roster, required briefing, first blind choice, contextual help return, and desktop/mobile map fallback. Commit `d25aca6` fixes a stale `#how-it-works` URL after the briefing/help return. A 2026-08-20 labelled disposable cloud project also passed live health, Hosting routing, disposable approved-account authentication, one Firestore-backed blind comparison, redaction, and count-only state inspection before deletion was requested. The complete five-identity visual record and broader behavioral-smoke coverage are still required before production deployment.
