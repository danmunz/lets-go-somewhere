...
function App() {
  ...
  if (screen === 'verdict' && results && user) return <>{journeyNav}<VerdictScreen results={results} currentUser={user} travelerName={travelerName} avatarFor={(id) => travelerById(id).image} onOpenMyResults={() => void openMyResults()} focusHeading={journeyFocus?.destination === 'verdict' ? journeyFocus.token : undefined} />{notice}{help}</>;
