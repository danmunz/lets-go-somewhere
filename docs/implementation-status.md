...
2. **Rehearsal — partially passed locally:** The isolated Auth/Firestore Emulator verifies configuration, concurrent/stale pending claims, racing immutable reveal creation, persisted snapshot reload, immutable final-decision conflicts, and redacted v2-result serialization. It now also drives five disposable authenticated identities through mismatch recovery, resume/duplicate handling, all five exact 32-choice rounds, completion gates, Dan-only reveal, immutable snapshot parity, and stale-tab final-decision behavior. A deliberately DEV-only browser Auth Emulator route is available for the remaining literal browser screenshots, desktop/mobile/fallback checks, keyboard and reduced-motion review; do not treat the automated API rehearsal as those visual records.
