...
- The independent transparent-social-ballot audit (`fbae847`) and guarded count-only preflight/reset implementation (`4ce95f0`) have passed their focused local checks. They do not replace the expanded Firestore Emulator proof, five-identity browser rehearsal, visual review, or advanced-model promotion gate.
- The operator procedure is drafted in `docs/one-trip-runbook.md`. The Firestore Emulator now runs locally, but the expanded emulator release proof and five-identity rehearsal still require recorded evidence. The reset refuses any started journey, so behavioral smoke testing must use a separately provisioned disposable Firebase/GCP environment; production preflight is count-only and must remain empty until the real trip starts.
